...
const VALIDATE_TIMEOUT_ERROR = "Your fallback argument is not of type number, you have been give a default time value, this may cause your logic not to work as expected!";
const validateTimeout = (time, fallback) => {
    if (typeof fallback !== "number")
        console.warn(VALIDATE_TIMEOUT_ERROR);
    const _fallback = typeof fallback === "number" ? fallback : DEFAULT_TIMER_FALLBACK;
...
*/
const raceSeekAndExecute = ({ query, callback, interval, timeout, errorCallback = () => null, }) => {
    let _interval = undefined;
    let _timeout = undefined;
    const run = typeof query === "string" &&
